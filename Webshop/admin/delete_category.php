...
<?php
    ...
    include "scripts/php/deleteCat.php";
    
    session_start();

    if(isset($_GET['delete_cat_id'])) {
        $_SESSION['remove'] = $_GET['delete_cat_id'];
        deleteCat();
    }
...
<html ng-app="webShopApp">
    ...
    <body>
        ...
        <div class="cart_wrapper" ng-controller="cat_brand_ControllerAdmin">
            <div ng-repeat="pro in categories track by $index">
                <div class="row custom_cart">
                    <div class="col-lg-10">
                        <a href="details.php?pro_id={{pro.ProductID}}"><h2>{{pro.CatName}}</h2></a>
                    </div>
                    <div class="col-lg-2">
                        <a href="delete_category.php?delete_cat_id={{pro.CatID}}"><i class="fa fa-times"></i></a>
                    </div>
                </div>
            </div>
        </div>
