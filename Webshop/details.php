...
<body>
    ...
    <div ng-controller="products_Controller" ng-init="proInit('<?php echo htmlentities(json_encode($proID))."', '".htmlentities(json_encode($proName))."', '".htmlentities(json_encode($proPrice))."', '".htmlentities(json_encode($proDesc))."', '".htmlentities(json_encode($proImage)) ?>')">
        <div class="content_area col-lg-12">
            <div class="detTitle">
                <h1>{{name}}</h1>
            </div>
            <div class="detImg col-lg-6">
                <img src="admin/images/product_images/{{image}}" alt="product_img" width="500px" height="500px">
            </div>
            <div class="detDesc">
                <p>{{desc}}</p>
            </div>
            <div class="detEnd">
                <h3 class="detPrice">{{price}} kn</h3>
                <?php if(isset($_SESSION['customer_email'])) : ?>
                    <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
                        ...
                        <input type="hidden" name="item_name" value="<?php echo $proName?>">
                        <input type="hidden" name="amount" value="<?php echo $proPrice*0.134814007?>">
                        ...
                        <input class="addCart_img" type="image" src="http://www.paypalobjects.com/en_US/i/btn/x-click-but22.gif" border="0" name="submit" width="87" height="23" alt="Make payments with PayPal - it's fast, free and secure!">

                    </form>
                <?php else : ?>
                    <p>Prijavite se ili registrirajte da bi mogli kupovati!</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <script src="scripts/js/product_detail_Controller.js"></script>
</body>
